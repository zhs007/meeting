...
class PCMError(ValueError):
    """Raised when raw PCM bytes do not match the required wire format."""
...
def validate_input_chunk(chunk: bytes) -> bytes:
    ...
    if len(chunk) != expected:
        raise PCMError(
            "Gemini Live Translation input chunk must be exactly "
            f"{expected} bytes for 16kHz mono PCM16 100ms audio; got {len(chunk)} bytes."
        )
    return chunk
